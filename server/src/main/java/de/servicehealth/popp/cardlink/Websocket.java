...
public class Websocket {
        ...
        try {
            ...
            if(jsonObject.containsKey("type")) {
                ...
                if(type.equals("registerEGK")) {
                    ...
                    store.registerEGK(tlsCertCN, webSocketSession, parseEGKPayload(Base64.getDecoder().decode(jsonObject.getString("payload"))), cardSessionId);
                    ...
                    if(entry.isPresent()) {
                        if(correlationId != null) {
                            ...
                            JsonObject sendAPDUResponse = Json.createReader(new ByteArrayInputStream(decodedPayload)).readObject();
                            
                            entry.get().getApduResponses().get(correlationId).complete(sendAPDUResponse.getString("response"));
                        } else {
                            LOG.warning("No correlationId provided for sendAPDUResponse");
                        }
                    } else {
                        LOG.warning("No entry found for tlsCertCN and cardSessionId: " + tlsCertCN + ", " + cardSessionId);
                    }
                }
            } else {
    ...
    public void onSend(@Observes SendApduPayloadWithSession event) {
        ...
        if (session != null && session.isOpen()) {
            try {
                ...
                if(entry.isPresent()) {
                    ...
                    LOG.warning("No entry found for tlsCertCN and cardSessionId: " + event.tlsCertCN + ", " + event.cardSessionId);
                }
            } catch (Exception e) {
