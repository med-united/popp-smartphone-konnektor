    ...
    try {
      VSDServicePortType vsdServicePortType =
          WebsocketTest.create(
              () -> new VSDService().getVSDServicePort(),
              "https://localhost:9443/services/VSDService/v5.2");
      ...
      String cardSessionId =
          msg.getMessage().getParameter().stream()
              .filter(s -> "CardHandle".equals(s.getKey()))
              .findFirst()
              .orElseThrow(
                  () ->
                      new IllegalArgumentException("Missing CardHandle parameter in event message"))
              .getValue();
      ...
      var cardlinkAVSWebSocketClientEndpoint =
          new CardlinkAVSWebSocketClientEndpoint(cardSessionId);
      client.connectToServer(
          cardlinkAVSWebSocketClientEndpoint, new URI("wss://localhost:9443/websocket/null"));
      Thread.sleep(
          5000); // Wait for WebSocket communication to complete before counting down the latch
