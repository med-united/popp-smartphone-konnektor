...
public class CardlinkPhoneWebSocketClientEndpoint {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(CardlinkPhoneWebSocketClientEndpoint.class);
  ...
  private void sendConfirmSMSCodeAndRegisterEGK(Session session, String smsCode)
      throws IOException {
      ...
      if (value instanceof JsonArray jsonArray) {
        ...
        if ("sendAPDU".equals(type)) {
          ...
          String websocketMessage =
              "[{\"type\": \"sendAPDUResponse\", \"payload\": \""
                  + Base64.getEncoder()
                      .encodeToString(("{\"response\": \"" + responseBase64 + "\"}").getBytes())
                  + "\"}, \""
                  + cardSessionId
                  + "\", \""
                  + correlationId
          ...
          log.error(
              "ERROR: {}",
              new String(Base64.getDecoder().decode(firstMessage.getString("payload"))));
          try {
            session.close();
          } catch (IOException e) {
  ...
  private static String decodePayload(JsonObject firstMessage) {
    ...
    String payloadDecoded =
        payloadLog.equals("null")
            ? "null"
            : new String(Base64.getDecoder().decode(payloadLog), StandardCharsets.UTF_8);
    String result =
        firstMessage
            .toString()
            .replaceFirst("(\"payload\":\\s*\")([^\"]+)(\")", "$1" + payloadDecoded + "$3");
    return result;
  }
  ...
  public static void prettyXml(JsonValue bundle) {
    try {
      ...
      String xml =
          StringEscapeUtils.unescapeJson(jsonEscaped.substring(1, jsonEscaped.length() - 1));
