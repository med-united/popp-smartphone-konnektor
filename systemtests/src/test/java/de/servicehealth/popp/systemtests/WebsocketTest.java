...
public class WebsocketTest<T> {
	...
	public void registerEgK() {
		...
		try {
			EventServicePortType eventServicePortType = create(() -> new EventService().getEventServicePort(),"https://localhost:9443/services/EventService/v7.2");
			...
			if(emulateCard) {
				...
				client.connectToServer(cardlinkWebSocketClientEndpoint, new URI("wss://localhost:9443/websocket/null"));
			}
			...
			try {
				...
				if(emulatePoPPClient) {
					...
					CardServicePortType cardService = create(() -> new CardService().getCardServicePort(),"https://localhost:9443/services/CardService/v8.2");
